...
departement_r1 <- read_excel("~/Schreibtisch/Presidentielle_2017_Resultats_Tour_1_c.xls", 
                                                     sheet = "Circo. Leg. Tour 1", skip = 2)

departement_r2 <- read_excel("~/Schreibtisch/Presidentielle_2017_Resultats_Tour_2_c.xls", 
                                                     sheet = "Circo. Leg. Tour 2", skip = 2)
departement_r1 <- dplyr::as_tibble(departement_r1)                    #verschönere den Datensatz 
colnames(departement_r1)[3:4] <- c("circ_code", "circ") #umbenennen die ersten 2 Spalten
departement_r1 <- departement_r1[,-c(1,2)]
departement_r1 <- dplyr::arrange(departement_r1, circ_code)    #sortiere den Datensatz gemaess 'departement_code'
...
departement_r1_new <- departement_r1[,1:22] 
departement_r1_new<- departement_r1_new[,-17] #ziehe die Grundspalten aus
...
for(i in 1:10){
  num <- 11:16 +7 * i 
  departement_r1_new2 <- dplyr::select(departement_r1, colnames(departement_r1)[c(1:15, num)])
...
surnames <- sort(levels(as.data.frame(departement_r1_simp)$surname))


################ Bearbeitung der Daten von der zweiten Runde ###################
departement_r2<- departement_r2[,-c(1,2)]
colnames(departement_r2)[1:2] <- c("circ_code", "circ")   #umbenennen die ersten 2 Spalten
departement_r2 <- dplyr::arrange(departement_r2, circ_code)

departement_comb <- dplyr::select(departement_r2, c(1,2,21,28))    #ziehe 'departement_code', 'departement' und gewonnene Stimmen von Macron und LePen aus 
...
tuned <- tuneMD(cbind(Macron, LePen, absr2, blancs_null2, reg_diff2) 
       ~ cbind(ARTHAUD, ASSELINEAU, CHEMINADE, `DUPONT-AIGNAN`, FILLON, HAMON, LASSALLE, 
               `LE PEN`, MELENCHON, MACRON, POUTOU, absr1, blancs_null1, reg_diff1), covariate = NULL, data=departement_comb, ntunes = 10, 
       totaldraws = 10000)
MD_departement <- ei.MD.bayes(cbind(Macron, LePen, absr2, blancs_null2, reg_diff2) 
                              ...
                              ,data = departement_comb,burnin=5000,sample=5000,thin=5,ret.beta="d",verbose=1,tune.list = tuned)

EI_departement <- ei.reg(cbind(Macron, LePen, absr2, blancs_null2, reg_diff2) 
                              ~ cbind(ARTHAUD, ASSELINEAU, CHEMINADE, `DUPONT-AIGNAN`, FILLON, HAMON, LASSALLE, 
                                      `LE PEN`, MELENCHON, MACRON, POUTOU, absr1, blancs_null1, reg_diff1)
                              ,data = departement_comb)


print(MD_departement)
plot(MD_departement)

lambda <- lambda.MD(MD_departement,columns=c("Macron","LePen","absr2", "blancs_null2"))
lasummary(lambda)
plot(lambda)


lambda_EI <- lambda.reg(EI_departement,columns=c("Macron","LePen","absr2", "blancs_null2"))
